...
\vspace{0.5em}
\noindent
Powyższe wyszukiwanie używa pomocniczych funkcji \texttt{tracePath} oraz \texttt{constructPath}, które służą odpowiednio do wyznaczenia listy wierzchołków znajdujących się na ścieżce na podstawie słownika \texttt{parent} i zbudowania ścieżki na podstawie listy wierzchołków \texttt{L}. Są one zdefiniowane następująco:\\

\begin{tcolorbox}[title=Budowanie ścieżki na podstawie wierzchołków]
\begin{verbatim}
tracePath(parent, s, t):
    L <- pusta lista
    dodaj t na koniec L
...
\vspace{0.5em}
\noindent
Przeszukiwanie wszerz jest sposobem przeglądania grafu zaproponowanym przez Edmondska i Karpa w swoim algorytmie będącym modyfikacją algortmu Forda-Fulkersona.

\subsubsection{Wyznaczanie przepływu maksymalnego - TODO FIX (z lab3)}
...
\newpage
\section{Spis załączonej płyty CD}
