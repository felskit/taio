...
\vspace{0.5em}
\noindent
Ważnym jest, że jednostki czasu są niepodzielne i zarówno ogólny czas przeznaczony na~wykonanie wszystkich projektów, jak i czas realizacji poszczególnych projektów musi wynosić przynajmniej jedną jednostkę czasu.\\
...
\subsection{Algorytm}
% opisać naturę naszego problemu (scheduling, NP trudny) i że świetnie nadaje się do tego algorytm genetyczny
% opisać co będzie genami, chromosomami (osobnikami z jednym chromosomem) i populacją
Wybranie uogólnienie problemu jest optymalizacyjnym problemem planowania. Problemy tego typu są~jednym z~klasycznych, występujących w literaturze przypadków zastosowania algorytmów genetycznym (przykładem jest praca \cite{wall1996}).\\
...
Załóżmy, że~dany jest następujący rozkład projektów w~oknie czasowym długości~6 przewidzianym na~ich wykonanie:
...
% powiedzieć, że losujemy całkowicie poprawną populację (bo funkcja z pełnym randomem raczej na pewno wyleci)

\subsubsection{Funkcja przystosowania}
% 1. jak sprawdzamy poprawnośc osobnika (badamy czy żaden projekt nie wyjeżdża poza granicę czasową)
% 2. jak szukamy przedziałów czasowych i wyznaczamy projekty do nich należące (zamiatamy początki i końce)
% !!! nie wiem czy tutaj już pisać dlaczego tak można, czy może jakiś krótki dowodzik jak w podstawowej wersji
% 3. wrzucić tutaj od razu info o przerabianiu interwału na ProblemData
% 4. oczywiście jak nie przejdzie walidacji to nie odpalamy nawet punktu 2 (zwracamy -1 i potem wyleci w pizdu)
% 5. powiedzieć, jak dla membera wygląda cały proces - jakis schemat z powyższych klocków - czyli: walidujemy, szukamy przedziałów, odpalamy solver na każdym i sumujemy łączne braki (uwzględniając długość przedziału)
% 6. zapisujemy też przyporządkowania, żeby nie liczyć bez sensu drugi raz potem
...
% jeżeli jakimś cudem populacja zginie (np. przez złe parametry mutacji) i wszystkie wylecą, to kończymy
...
n_point_crossover(parent1, parent2, n):
    ...
    len <- długość osobnika
    cuts <- tablica n liczb wylosowanych z zakresu [1, len - 1]
    dodaj len na koniec cuts
...
\begin{center}
	cuts = [1, 3, 4, 6],
\end{center}
ponieważ długość osobnika wynosi w tym przypadku 6. Pary \texttt{(cut\_from, cut\_to)}, czyli granice odcinków wymiany genów, które możemy tutaj wyróżnić to \texttt{(1, 3)} oraz \texttt{(4, 6)}. Jeżeli liczba elementów \texttt{cuts} byłaby nieparzysta, ostatni element zostałby pominięty, ponieważ nie utworzyłby z innym pary.\\
...
Drugim operatorem genetycznym biorącym udział w procesie ewolucji jest mutacja. Pewien osobnik poddawany jest zamianie \texttt{n} losowych genów na inne -- także losowe.\\
...
n_point_mutation(member, n):
    ...
    jeżeli przeprowadzenie mutacji nie ma sensu:
        zwróć mutated
    len <- długość osobnika
    mutation <- tablica n liczb z przedziału [0, len - 1]
...
\vspace{0.5em}
\noindent
Podobnie jak w przypadku operacji krzyżowania, nowy osobnik zostanie dodany do populacji tylko, jeżeli jeszcze w niej nie występuje. Szansa na to, że dany osobnik zostanie poddany mutacji jest także parametrem algorytmu genetycznego, który zależy od rozwiązywanego zadania i jego rozmiaru.
