...
$$ \assign : E \to P \times U $$
gdzie dla projektu $p \in P$ i~umiejętności $u \in U$ zachodzi
$\assign(e) = (p,u)$ wtedy i~tylko wtedy, gdy:
\begin{itemize}
...
$$ \left| \assign^{-1}[p,u] \right| \leq \need(p,u) $$
gdzie $\assign^{-1}[p,u]$ jest przeciwobrazem funkcji $\assign$ dla argumentów
$p,s$.
\end{defn}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		$u \in U, p \in P$ określamy wartość przyporządkowania $\assign_f$ jako
		$$ (\forall e \in E_{u,p}) \assign_f(e) = (u,p) $$
	...
	\begin{itemize}
		...
		taka sama, każdego eksperta w~dziedzinie umiejętności $u$
		...
		$f(u,p) = \left|\assign_f^{-1}[u,p]\right|$. Stąd na~mocy definicji
		przepływu mamy
		$$ \left|\assign_f^{-1}[u,p]\right| = f(u,p) \leq c(u,p) = \need(u,p)$$
	...
	\left(\sum_{p \in P} \sum_{u \in U} \left| \assign_{\max}^{-1} [p,u]\right|\right) 
	&= M(\assign_{\max}) > \\
	&> \left( \sum_{p \in P} \sum_{u \in U} \need(p,u) \right) - |E|
	\end{align*}
	...
	\begin{enumerate}
		\item Jeżeli $(\forall u \in U) \ability(e,u) = 0$, to eksperta
		nie~można przyporządkować do~żadnego projektu.
		\item Załóżmy, że istnieje umiejętność $u \in U$ taka, że
		$\ability(e,u) = 1$.
		\begin{enumerate}
			\item Jeżeli $(\forall p \in P) \need(p,u) =
			\left|\assign_{\max}^{-1} [p,u]\right|$, to eksperta nie~można
			przyporządkować do~żadnego projektu.
			\item Załóżmy, że~istnieje projekt $p$ taki, że
			$\need(p,u) > \left|\assign_{\max}^{-1} [p,u]\right|$.
			Wówczas w~sieci rezydualnej dla przepływu $f_{\max}$ istnieje
			ścieżka rozszerzająca postaci $s e u p t$, wzdłuż której można
